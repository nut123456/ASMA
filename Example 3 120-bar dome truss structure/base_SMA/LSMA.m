...
z=0.03; % parameter
threshold_iter = round(Max_iter / 2); % Set a threshold halfway through the iterations
...
while  it <= Max_iter
    ...
    for i = 1:N
        ...
        for j=1:dim
            ...
            if r1 < p
                    X(i,j) = bestPositions(j) + vb(j) * (((weight(i,j) * bestPositions2(j) - X(A,j)) + (weight(i,j) * bestPositions3(j) - X(B,j))));
            else
                    X(i,j) = vc(j) * X(i,j);
            end
         end
