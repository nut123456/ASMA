...
Convergence_curve_ASMA_all = zeros(Times, 250);
Convergence_curve_AO_all = zeros(Times, 250);
Convergence_curve_SMA_all = zeros(Times, 250);
...
for i = 1:Times
    ...
    [Destination_fitness_ASMA(i), bestPositions_ASMA(i,:), Convergence_curve_ASMA(i,:)] =ASMA(N, 6250, lb, ub, dim, fobj);
...
for i = 1:Times
    %[bestfitness,Leader_pos,Convergence_curve]=ASMA(N,MaxFEs,lb,ub,dim,fobj)
    [Destination_fitness_AO(i), bestPositions_AO(i,:), Convergence_curve_AO(i,:)] =AO(N, 6250, lb, ub, dim, fobj);
...
for i = 1:Times
    %[bestfitness,Leader_pos,Convergence_curve]=ASMA(N,MaxFEs,lb,ub,dim,fobj)
    [Destination_fitness_SMA(i), bestPositions_SMA(i,:), Convergence_curve_SMA(i,:)] =SMA(N, 250, lb, ub, dim, fobj);
...
current_ylim = ylim;
ylim([current_ylim(1)*0.85, 15000]);  % Set upper y-limit to 3000
% Save the figure
saveas(gcf, 'convergence_curves_Dome_120.png');
