...
Times = 3; % Number of independent times you want to run the SMA, LSMA, CLSMA
display(['Number of independent runs: ', num2str(Times)]);
Convergence_curve_ASMA_all = zeros(Times, 500);
Convergence_curve_AO_all = zeros(Times, 500);
Convergence_curve_SMA_all = zeros(Times, 500);
...
for i = 1:Times
    %[bestfitness,Leader_pos,Convergence_curve]=ASMA(N,MaxFEs,lb,ub,dim,fobj)
    [Destination_fitness_SMA(i), bestPositions_SMA(i,:), Convergence_curve_SMA(i,:)] =SMA(N, 500, lb, ub, dim, fobj);
...
current_ylim = ylim;
ylim([current_ylim(1)*0.85, 600]);  % Set upper y-limit to 3000
% Save the figure
